...
class VariableEnemy:
    ...
    def update(self):
        ...
        if self.isDead == True:
            ...
            self.playerObj.dungeonObj.returnCurrentRoom().enemylist.remove(
                self)
        if self.dot == True:
    ...
    def collision(self, obj):
        """distance formula"""
        if math.sqrt(pow(self.x - obj.x, 2) +
                     pow(self.y - obj.y, 2)) <= self.range:
            return True
        ...
        if self.drawDifferent:
            ...
            for sprite in self.spriteList:
                Display.DISPLAYSURF.blit(
                    pygame.transform.scale(sprite, (self.size, self.size)),
