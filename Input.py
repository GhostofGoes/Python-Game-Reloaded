...
class Input:
    ...
    def update(self, playerObj, menuObject):
        ...
        for event in pygame.event.get():
            ...
            # TODO: non 10-button controller handling
                ...
                elif self.controllers[0].get_button(
                        6):  # 'back' (the double window button thing)
                    action = self.action_mappings["JOY_BACK"]
                elif self.controllers[0].get_button(
                        7):  # 'start' (triple lines button thing)
                    action = self.action_mappings["JOY_START"]
                elif self.controllers[0].get_button(
                        8):  # 'LS' depression (maybe)
                    action = self.action_mappings["JOY_LS"]
                elif self.controllers[0].get_button(
                        9):  # 'RS' depression (maybe)
            ...
            self._perform_action(
                action=action, playerObj=playerObj, menuObject=menuObject)

            if pygame.mouse.get_pressed():
                self._perform_action(self.action_mappings["MOUSE_PRESSED"],
                                     playerObj, menuObject)
    ...
    def _axis_movement(self, con, playerObj):
        ...
        if self.controllers[con].get_numaxes() == 5:
            ...
            self._log.error('Improper number of axis on controller [%s] %s.',
                            con, self.controllers[con].get_name())
            return
        ...
        if self.controllers[con].get_axis(
            ...
            playerObj.direction = 'left'
                ...
                LS_X) > POS_SENS:  # Left stick, X-axis right
            playerObj.moveLeft = False
            playerObj.moveRight = True
            playerObj.direction = 'right'
        elif NEG_SENS < self.controllers[con].get_axis(
                LS_X) < POS_SENS:  # Reset left stick, X-axis
            playerObj.moveLeft = False
            playerObj.moveRight = False
            playerObj.currentWeapon.spriteObj.resetSpriteList()
        if self.controllers[con].get_axis(
                LS_Y) < NEG_SENS:  # Left stick, Y-axis up
            playerObj.moveUp = True  # These are backward, don't ask me why
            playerObj.direction = 'up'
        elif self.controllers[con].get_axis(
                LS_Y) > POS_SENS:  # Left stick, Y-axis down
            playerObj.moveDown = True
            playerObj.direction = 'down'
        elif NEG_SENS < self.controllers[con].get_axis(
                LS_Y) < POS_SENS:  # Reset left stick, Y-axis
            playerObj.moveUp = False
            playerObj.moveDown = False
            playerObj.currentWeapon.spriteObj.resetSpriteList()
        ...
        if self.controllers[con].get_numaxes() == 5:
            ...
            if self.controllers[con].get_axis(
                    2) < NEG_SENS:  # Left trigger,  resting state, reset stuff
                pass
            elif self.controllers[con].get_axis(
                    2) > POS_SENS:  # Left trigger pulled
                pass

            if self.controllers[con].get_axis(
                    3) < NEG_SENS:  # Right trigger, resting state, reset stuff
                playerObj.isAttacking = 0  # TODO: Why not True? Et tu?
            elif self.controllers[con].get_axis(
                    3) > POS_SENS:  # Right trigger pulled
                playerObj.isAttacking = 1

        # Right Stick
        if self.controllers[con].get_axis(
                RS_Y) >= POS_SENS:  # Right stick, Y-axis down
            self.vPosY[con] += POS_CHANGE_PER_TICK
        elif self.controllers[con].get_axis(
                RS_Y) < POS_SENS:  # Right stick, Y-axis up
            self.vPosY[con] -= POS_CHANGE_PER_TICK
        if self.controllers[con].get_axis(
                RS_X) >= POS_SENS:  # Right stick, X-axis right
            self.vPosX[con] += POS_CHANGE_PER_TICK
        elif self.controllers[con].get_axis(
                RS_X) < POS_SENS:  # Right stick, X-axis left
            self.vPosX[con] -= POS_CHANGE_PER_TICK
