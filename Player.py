                    ...
                    if projectile.exists == True:
                        if functions.objCollision(projectile, enemy):
                            ...
                            print("%s hit for %s damage!" %
                                  (enemy.name, projectile.damage))
                            if enemy.health <= 0:
                                enemy.death()
                                self.experience += 4
                                print("%s gained 4 Exp!" % (self.name))
                                self.rangeExp += 2
                                print("%s has gained 2 ranged exp!" %
                                      (self.name))
    ...
    def updateAttackSprite(self):
        ...
        if self.direction == 'left':
            self.attackRect = pygame.Rect(
                self.x - self.currentWeapon.range + self.width,
                self.y - self.currentWeapon.range + self.width,
                self.currentWeapon.range, self.currentWeapon.range)
            self.currentWeapon.spriteObj.update(
                self.x - self.currentWeapon.range + self.width,
                self.y - self.currentWeapon.range + self.width, False, 0)
        elif self.direction == 'right':
            self.attackRect = pygame.Rect(self.x, self.y,
                                          self.currentWeapon.range,
                                          self.currentWeapon.range)
            self.currentWeapon.spriteObj.update(self.x, self.y, False, 0)
        elif self.direction == 'up':
            self.attackRect = pygame.Rect(
                self.x, self.y - self.currentWeapon.range + self.height,
                self.currentWeapon.range, self.currentWeapon.range)
            self.currentWeapon.spriteObj.update(
                self.x, self.y - self.currentWeapon.range + self.height, False,
                0)
        elif self.direction == 'down':
            self.attackRect = pygame.Rect(
                self.x - self.currentWeapon.range + self.width, self.y,
                self.currentWeapon.range, self.currentWeapon.range)
            self.currentWeapon.spriteObj.update(
                self.x - self.currentWeapon.range + self.width, self.y, False,
                0)
